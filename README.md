...
- `/events/`：重大外部事件的質化比較（發文原因、行動與原始證據）
...
- `scripts/classify_context.py` 另外判斷發文觸發原因（自主議程、外部事件、直接回應、例行發布、待確認）與
  貼文行動（政策倡議、資訊轉達、行政進度、澄清、批評等）。觸發原因和行動是兩個維度，因此颱風貼文可以同時是
  「外部事件驅動」與「政策倡議」。分類保留信心、判斷證據、規範版本及人工複核狀態。
- `data/sources/events.json` 是人工維護的事件期間與關鍵字；`scripts/build_qualitative.py` 產生事件比較 API、
  質化統計及議題選擇器使用的候選人自主政策倡議向量。
...
- [x] 正規化、去重、議題分類
- [x] 多維貼文分類、事件比較頁、候選人頁原因／行動篩選
- [x] 議題選擇器（以自主政策倡議貼文為依據，附來源與資料不足提示）
- [ ] 完成 200 篇雙人標註與裁決，達到觸發原因正確率 80%、直接回應 precision 90% 的品質門檻
...
python3 scripts/context_annotation.py export  # 產生 200 篇分層人工標註表
python3 scripts/context_annotation.py score   # 評估完成裁決的標註表；未達門檻會回傳非零
